...
import { 
  X, Sparkles, ThumbsUp, Eye, Clock, CheckCircle2, 
  ExternalLink, Copy, Check, ChevronDown, ChevronUp, 
  Share2, Play, Bell, Info, ShieldCheck, Tag, Music, Flame, Zap
} from 'lucide-react';
...
interface YouTubeVideoDescriptionSheetProps {
  isOpen: boolean;
  onClose: () => void;
  track: Track | null;
  realChannelName?: string;
  realChannelAvatar?: string;
  realSubscriberCount?: string;
  realLikeCountStr?: string;
  realViewCount?: string;
...
export const YouTubeVideoDescriptionSheet: React.FC<YouTubeVideoDescriptionSheetProps> = ({
  isOpen,
  onClose,
  track,
  realChannelName,
  ...
  // Channel subscriptions state for featured channels
  ...
  useEffect(() => {
    if (isOpen && track?.id) {
      ...
      setShowAiCard(false);
      setAiSummary(null);
      fetchFullVideoDetails(track.id);
    }
  }, [isOpen, track?.id]);

  const fetchFullVideoDetails = async (videoId: string) => {
    ...
    try {
      ...
      if (data.metadata && data.metadata[videoId]) {
        const meta = data.metadata[videoId];
        setRawDescription(meta.description || '');
      } else {
        // Fallback default description
        setRawDescription(generateDefaultDescription(track));
      }
    } catch {
      setRawDescription(generateDefaultDescription(track));
    } finally {
  ...
  const generateDefaultDescription = (t: Track | null) => {
    if (!t) return '';
    return `Hello guys, is video me humne ${t.channel || 'creator'} ke sath ek mazedaar video stream banaya hai.

Enjoy official high-definition audio & video stream for "${t.title}".

Download App For Unlimited Music & Video Streams:
https://play.google.com/store/apps

My Official Channels:
• @${(t.channel || 'artist').toLowerCase().replace(/[^a-z0-9]/g, '')}
• @${(t.channel || 'artist').toLowerCase().replace(/[^a-z0-9]/g, '')}Vlogs
• @${(t.channel || 'artist').toLowerCase().replace(/[^a-z0-9]/g, '')}Shorts

Follow on Social Media:
Instagram: https://instagram.com/${(t.channel || 'artist').toLowerCase().replace(/[^a-z0-9]/g, '')}
Facebook: https://facebook.com/${(t.channel || 'artist').toLowerCase().replace(/[^a-z0-9]/g, '')}

#${(t.genre || 'music').replace(/\s+/g, '').toLowerCase()} #youtube #official #${(t.channel || 'creator').replace(/[^a-z0-9]/g, '').toLowerCase()}`;
  };

  const handleToggleChannelSub = (chId: string, chName: string) => {
    setSubscribedChannels(prev => {
      const nextState = !prev[chId];
      if (nextState) {
        onShowToast?.(`Subscribed to ${chName}!`, 'success');
      } else {
        onShowToast?.(`Unsubscribed from ${chName}`, 'info');
      }
      return { ...prev, [chId]: nextState };
    });
  };
    ...
    try {
      const res = await fetch('/api/youtube/summarize', {
        ...
        body: JSON.stringify({
          videoId: track.id,
          title: track.title,
          channel: realChannelName || track.channel,
          description: rawDescription || track.title
        })
      });
      const data = await res.json();
      setAiSummary(data);
      ...
      setAiSummary({
        summary: `"${track.title}" by ${realChannelName || track.channel} provides an engaging music video stream with high fidelity audio and authentic creator moments.`,
        keyPoints: [
        ...
        tags: ["Music", "Official Stream", "Entertainment"]
      });
    } finally {
  ...
  const formattedViews = realViewCount?.replace(' views', '') || (track.views ? `${track.views}` : '359,071');
  const formattedTimeAgo = publishedTimeAgo || track.publishedTime || '4h';

  // Featured channels mock/parsed list
  const featuredChannels: FeaturedChannel[] = [
    {
      id: 'ch-main',
      name: displayChannel,
      handle: channelHandle,
      subscribers: realSubscriberCount || '2.57M subscribers',
      category: 'My Official Channel',
      avatarUrl: realChannelAvatar || `https://api.dicebear.com/7.x/identicon/svg?seed=${encodeURIComponent(displayChannel)}`
    },
    {
      id: 'ch-vlog',
  ...
  // Text parser for clickable links, hashtags, and timestamps
  const renderFormattedDescription = (text: string) => {
    const lines = text.split('\n');
    return lines.map((line, lineIdx) => {
      // Regex to split by URLs or hashtags or timestamps
      ...
      return (
        <p key={lineIdx} className="min-h-[1.25rem] leading-relaxed">
          {parts.map((part, partIdx) => {
            ...
            // Timestamp seek detection (e.g. 01:24)
          ...
          <div className="px-4 sm:px-5 py-4 overflow-y-auto space-y-4 flex-1 overscroll-contain pb-24">
            ...
            <div className="bg-[#181a20] border border-white/5 rounded-2xl p-4 sm:p-4.5 space-y-3 shadow-md">
              <div className={`text-xs sm:text-[13px] text-zinc-300 space-y-2.5 transition-all ${isExpanded ? '' : 'line-clamp-4 max-h-28 overflow-hidden'}`}>
                {renderFormattedDescription(rawDescription || generateDefaultDescription(track))}
              </div>
            ...
            {/* FEATURED / RELATED CHANNELS (As in Screenshot 2) */}
            <div className="space-y-3 pt-1">
              {featuredChannels.map((chan) => {
                const isSub = subscribedChannels[chan.id] ?? (chan.id === 'ch-main' ? isSubscribed : false);
                return (
                  <div key={chan.id} className="space-y-1.5">
                    ...
                    <div className="bg-[#181a20] hover:bg-[#20232b] border border-white/5 rounded-2xl p-3.5 flex items-center justify-between gap-3 transition-colors shadow-sm">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (chan.id === 'ch-main' && onToggleSubscribe) {
                            onToggleSubscribe(displayChannel);
                          } else {
                            handleToggleChannelSub(chan.id, chan.name);
                          }
                        }}
